...
fn draw_sidebar(cwd: str, open_file: str, ft_sel: int, ft_scroll: int,
                git_out: str, ctx_row: int, ctx_x: int, ctx_y: int,
                mx: int, my: int, top_y: int, bot_y: int) {
    let fh   = gui_font_height();
    let sw   = L_SIDEBAR();
    let row_h = 22;
    ...
    // Background
    c_sidebar();
    gui_fill_rect(0, top_y, sw, bot_y - top_y);

    // Header
    c_titlebar();
    gui_fill_rect(0, top_y, sw, 24);
    c_dim();
    gui_draw_text("EXPLORER", L_MARGIN(), top_y + (24 - fh) / 2);

    // Workspace label (cwd basename) with mini git dirty indicator
    let workspace = path_basename(cwd);
    c_white();
    gui_draw_text(workspace, L_MARGIN(), top_y + 28);
    // Show git dirty marker
    let is_dirty_ws = if str_len(git_out) > 0 { 1 } else { 0 };
    if is_dirty_ws == 1 {
        c_git_m();
        gui_draw_text("*", L_MARGIN() + gui_text_width(workspace) + 4, top_y + 28);
    }

    // ".." parent dir entry
    let parent_y = top_y + 52;
    let par_hot  = in_rect(mx, my, 0, parent_y, sw, row_h);
    ...
    let visible  = (bot_y - list_top) / row_h;
    ...
    while i < visible {
        ...
        let is_hot = in_rect(mx, my, 0, row_y, sw, row_h);
    ...
    // Scrollbar
    draw_scrollbar(sw - 11, list_top, bot_y - list_top, count + 1, visible, ft_scroll);

    // Right border
    c_border();
    gui_draw_line(sw, top_y, sw, bot_y);

    // Context menu (if visible)
    let ctx_items = "New File\nNew Folder\nRename\nDelete\nCopy Path";
    if ctx_row >= 0 {
        draw_context_menu(ctx_x, ctx_y, ctx_items, 5, mx, my);
    }
}
